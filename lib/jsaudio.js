...
// Helpers
const readWriteStreamArgs = (ref, stream, buffer, frames) => {
  if (!frames && Number.isInteger(buffer)) {
    frames = buffer
    buffer = null
  }
  frames = frames || ref.framesPerBuffer
  buffer = buffer || ref.buffer || ref.getBuffer()
  stream = stream || ref.stream || ref.getStream()
  return { stream, buffer, frames }
}
...
function JsAudio (opts) {
  ...
  opts.framesPerBuffer = opts.framesPerBuffer || opts.bufferSize || 1024
...
JsAudio.prototype.readStream = function (stream, buffer, frames) {
  let self = this
  let args = readWriteStreamArgs(self, stream, buffer, frames)
  JsAudioNative.readStream(args.stream, args.buffer, args.frames)
  return self
}

// Override inherited writeStream method
JsAudio.prototype.writeStream = function (stream, buffer, frames) {
  let self = this
  let args = readWriteStreamArgs(self, stream, buffer, frames)
  JsAudioNative.writeStream(args.stream, args.buffer, args.frames)
  return self
}

// Get info for all host APIs as array
JsAudio.prototype.getHostApiList = function () {
  ...
  self.hostApis = refAry.map((d) => JsAudioNative.getHostApiInfo(d))
